<?php
    ...
    function contractScraper($dex_arr){
        ...
        return array_filter(array_values(array_unique($contractArr)), function($value) { return !is_null($value) && $value !== ''; });          
    }
    ...
    function transactionLog($contract_arr){
        ...
        foreach ($contract_arr as $contractkey => $contractvalue) {
            ...
            if ( $result['errno'] != 0 )
               ...
               { print_r($result['errno']); print_r($get_tx_url); }
        ...
        return array_filter(array_values(array_unique($txArr)), function($value) { return !is_null($value) && $value !== ''; });   
    ...
    function userValidAddress($get_all_user_addr_arr){
        ...
        $get_all_user_addr_arr = array_splice($get_all_user_addr_arr, 0, 5000);        
        ...
        foreach ($get_all_user_addr_arr as $addresskey => $addressvalue) {
            ...
            if ( $result['errno'] != 0 )
               ...
               { print_r($result['errno']); print_r($get_addr_url); }
        ...
        return array_filter(array_values(array_unique($addressArr)), function($valuesd) { return !is_null($valuesd) && $valuesd !== ''; });
    ...
    function index(){
        ...
        $dex_url = 'https://etherscan.io/dextracker_txns';
        ...
        foreach ($nodes as $node) 
        {
            array_push($DexArr, $node->nodeValue);
        }
        ...
        $user_valid_address_arr = userValidAddress($get_all_user_tx_log_arr);
        ...
        if(count($user_valid_address_arr) > 0){
           ...
           return $final_data = array_map(function($finalvalues){   return [ 'address' => $finalvalues ];     }, $user_valid_address_arr);        
        }else{
...
if($new_file_record){
    ...
    echo 'LAST NAME' . $get_last_file_name =  get_last_file_name();
    ...
    $array_chunk = array_chunk($temp_file_line, 500);
    ...
    // print_r($array_chunk);
        ...
        if($array_chunk){
            foreach ($array_chunk as $chunkkey => $chunkvalue) {
                ...
                $title = 'etherscan_'. $time .'.json';
                echo $path = __DIR__ . '/eth/' . $title;
                file_put_contents($path, (json_encode($chunkvalue, JSON_PRETTY_PRINT))); //generate json file        
            }
        ...
        $array_chunk = array_chunk($new_file_record, 500);
